...
function mockPrototype() {
   ...
   */
  const getContext2D = _vitest.vi.fn(function getContext2d(type) {
    if (type === '2d') {
      /**
   ...
   */
  const toBlobOverride = _vitest.vi.fn(function toBlobOverride(callback, mimetype) {
   ...
   * jpeg size is variable, and so is png. TODO: Is there a better way to do this?
   */
  const toDataURLOverride = _vitest.vi.fn(function toDataURLOverride(type, encoderOptions) {
    switch (type) {
      case 'image/jpeg':
        break;
      case 'image/webp':
