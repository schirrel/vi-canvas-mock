...
export default vi.fn(function createImageBitmap(
  ...
  return new Promise((resolve, reject) => {
    ...
    if (length === 3 || length === 4)
      return reject(
        new TypeError(
          "Failed to execute 'createImageBitmap' on 'Window': Valid arities are: [1, 2, 5, 6], but " +
            length +
            ' arguments provided.'
        )
      );
    ...
    if (
      img instanceof Blob ||
      (img && img.constructor && img.constructor.name === 'Blob')
    )
      validImage = true;
    ...
    if (length >= 2) {
      ...
      if (options !== null && options !== void 0) {
        if (typeof options !== 'object')
          throw new TypeError(
            "Failed to execute 'createImageBitmap' on 'Window': parameter " +
              index +
